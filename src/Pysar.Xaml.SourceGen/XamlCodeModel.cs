...
{
    private const string PysarNs = "https://mriyalab.com/pysar";
    ...
    public string Emit(SourceProductionContext context)
    {
        ...
        if (RequiresRuntimeLoader)
            ...
            builder.Append(EmitConstructionOrFallBack(context));
        builder.AppendLine("        }");
    ...
    /// </summary>
    private string EmitConstructionOrFallBack(SourceProductionContext context)
    {
        try
        {
            return new XamlConstructionEmitter(Compilation, ResolveTypeFqn).EmitBody(RootElement, IsView);
        }
        catch (XamlGenException exception)
        {
            Report(
                context,
                "PQX013",
                $"Compiled XAML construction fell back to the runtime loader: {exception.Message}",
                DiagnosticSeverity.Warning);
            var builder = new StringBuilder();
            EmitRuntimeBody(builder);
            return builder.ToString();
    ...
    private static bool IsPysar(string xmlNamespace)
        => xmlNamespace == PysarNs || string.IsNullOrEmpty(xmlNamespace);
