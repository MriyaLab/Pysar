...
public class MasterDetailReport
{
    public Report Build()
    {
        ...
        {
            row.AddElement(BuildMonthHeader(month, lightGray, darkGray, negative));
            row.AddElement(BuildEntryColumnsHeader(muted));
            row.AddGroup(month.Entries, (entry, entryRow) =>
                entryRow.AddElement(BuildEntryRow(entry, lightGray, muted, negative)));
        });
        ...
        Color negative)
    {
        ...
        // compact so the detail starts on the same page. The year is end-aligned inside the only column.
        var titles = new Grid
        {
            Size = new Size(SizeLength.Fill, SizeLength.Auto),
        ...
        titles.AddElement(Label("Annual Report", 22, FontStyle.Bold, darkGray), 0, 0);
        titles.AddElement(Label(data.Company, 14), 1, 0);
        var year = Label(data.YearLabel, 14);
        year.HorizontalAlignment = Alignment.End;
        year.VerticalAlignment = Alignment.Center;
        titles.AddElement(year, 0, 1, 2, 1);
        ...
        atGlance.AddElement(EndCell(Money(data.Income), 9, FontStyle.Bold), 1, 1);
        atGlance.AddElement(Label("Total expenses:", 9), 1, 2);
        atGlance.AddElement(EndCell(Money(data.Expense), 9, FontStyle.Bold), 1, 3);
        ...
        atGlance.AddElement(EndCell(data.BestMonth, 9), 2, 1);
        atGlance.AddElement(Label("Weakest month:", 9), 2, 2);
        atGlance.AddElement(EndCell(data.WorstMonth, 9), 2, 3);
        atGlance.AddElement(Label("Margin:", 9), 2, 5);
        atGlance.AddElement(EndCell(data.MarginLabel, 9), 2, 6);

        var stack = new StackPanel { Size = Size.Auto };
    ...
    private static Grid BuildMonthHeader(
        MonthSummary month,
        Color lightGray,
        Color darkGray,
        Color negative)
    {
        ...
        var name = Label(month.Name, 9, FontStyle.Bold, darkGray);
    ...
    private static Grid BuildEntryRow(
        LedgerEntry entry,
        Color lightGray,
        Color muted,
        Color negative)
    {
        ...
        grid.AddElement(ItemCell(entry.Category, muted), 0, 0);
    ...
    private static Text ItemCell(string content, Color? muted = null)
    {
        var cell = muted is null ? Label(content, 9) : Label(content, 9, color: muted);
        cell.VerticalAlignment = Alignment.Center;
        return cell;
    }
